...
extract('Month',cast(date as date)) Month_No,extract('week',cast(date as date)) week_No,
count(distinct date) NoOfDays,
sum(new_cases)total_new_cases,
avg(new_cases) avg_new_cases,
sum(new_active_cases)total_active_new_cases,
avg(new_active_cases) avg_active_new_cases,
